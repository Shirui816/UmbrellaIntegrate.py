...
Umbrella Integration<sup>[1]</sup> algorithm for calculating PMF with Python.
...
Raw data was generated by [Gaussian distribution](https://en.wikipedia.org/wiki/Normal_distribution) for each window with `MEAN=window_center` and `STD=0.8`, the centers are in range of `0.0 ~ 19.5` by step of `0.5`, here is the result compared with WHAM<sup>[2]</sup>:
...
**Do not integrate over negative intervals with $x^3$ in $exp$**
